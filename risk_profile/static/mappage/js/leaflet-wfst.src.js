...
L.Format.GML = L.Format.Base.extend({
  ...
  generateLayer: function (feature) {
    ...
    if (!geometryField) {
		geometryField = feature.getElementsByTagName("gml:location")[0];
		if (!geometryField) {
		  console.log(
...
L.WFS = L.FeatureGroup.extend({
  ...
  describeFeatureType: function (successCallback, errorCallback) {
    ...
    L.Util.request({
      ...
      success: function (data) {
		  //console.log("describeFeatureType",data);
